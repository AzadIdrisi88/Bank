...
def bankdetail(request):
    ...
    accounts = bankModel.objects.filter(accountno=accno)
    if len(accounts) <= 0:
        return HttpResponse("Please Enter correct Account No")
    else:
        accounts = accounts[0]
        name = accounts.name
        ...
        print(accounts)
        detail = {}
        detail["accountno"] = accounts.accountno
        detail["name"] = accounts.name
        detail["balance"] = accounts.balance
